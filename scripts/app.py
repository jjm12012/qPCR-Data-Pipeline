...
st.markdown(
    "Upload one or more **CFX Maestro `.xlsx` exports** to run the full analysis pipeline. "
    "Results appear as an interactive dashboard with download options for all outputs."
)
...
if "results" in st.session_state:
    ...
    # ── Tabs ──────────────────────────────────────────────────────────────────
    tab_summary, tab_plates, tab_charts, tab_qc, tab_dl = st.tabs([
        "📊 Summary", "🧫 Plate Maps", "📈 Charts", "⚠️ QC Flags", "📥 Downloads",
    ])
    ...
    with tab_summary:
        ...
        key = "01_positivity_rate_bar.png"
        if key in res["figures"]:
            st.image(res["figures"][key],
                     caption="Positivity Rate by Sample Type",
    ...
    with tab_plates:
        plate_figs = {
            k: v for k, v in res["figures"].items()
            if Path(k).stem.startswith("05_") or Path(k).stem.startswith("06_")
        }
        if plate_figs:
            for name, img_bytes in sorted(plate_figs.items()):
                st.image(img_bytes, caption=_caption(name), use_container_width=True)
                st.divider()
        else:
            st.info("No plate map figures were generated.")

    # ── Charts tab ────────────────────────────────────────────────────────────
    with tab_charts:
        other_figs = {
            k: v for k, v in res["figures"].items()
            if not any(Path(k).stem.startswith(p) for p in ("01_", "05_", "06_"))
        }
        if other_figs:
            for name, img_bytes in sorted(other_figs.items()):
                st.image(img_bytes, caption=_caption(name), use_container_width=True)
                st.divider()
        else:
            st.info("No additional charts (some charts require 2+ runs).")
